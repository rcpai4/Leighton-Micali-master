...
f = open('inputfile_hlms', 'r')

class entropy_source(object):
...
def lmots_gen_priv():
    priv = list()
    for i in range(0, p):
        tmp = entropySource.read(n);
        #print "i: " + stringToHex(tmp)
        priv.append(tmp);
    return priv
...
def lmots_gen_sig(private_key, I, q, message):
    ...
    #print "CHECKSUM: " + stringToHex(checksum(hashQ))
...
class lms_private_key(object):
    ...
    def __init__(self):
        ...
        for q in range(0, 2**h):
            ...
            print "generating " + str(q) + "th OTS key PUBLIC KEY " + stringToHex(ots_pub)
        self.leaf_num = 0
        self.nodes = {}
    ...
    def sign(self, message):
        if (self.leaf_num >= 2**h):
            return ""
        #print "LMOTS SIG INP "
        #for i, e in enumerate(self.priv[self.leaf_num]):
        #    print "i(" +str(i) +"): \t" + stringToHex(e)
        #print "I: " + stringToHex(self.I)
        #print "LEAF NUM: " + str(self.leaf_num)
        #print "message: " + message
...
class hlms_private_key(object):
    ...
    def init_level_2(self):
        ...
        print_lms_sig(self.sig1)
        #print "SIG1 : " + stringToHex(self.sig1);
        #sys.exit(1);
...
class hlms_public_key(object):
    ...
    def verify(self, message, sig):
        pub2, sig1, lms_sig = decode_hlms_sig(sig)
        print "pub2: " + stringToHex(pub2)
        print "sig1: " + stringToHex(sig1)
        print "lms_sig: " + stringToHex(lms_sig)
        print "pub1: " + stringToHex(self.pub1.value)
